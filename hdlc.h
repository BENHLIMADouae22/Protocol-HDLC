...
//cette fct extrait une sous chaine de mot, et la met dans res
...
//cette fct enlève les zeroes inutiles d'un mot binaire, exemple: 0110 devient 110
...
//fct qui ajoute des zeroes à droite ou à gauche d'un mot binaire, selon dir
...
void SNWexchange()
{
    ...
    if(pid=fork())
    {
        puts("Je suis le processus père, je simule l'émetteur, mon fils sera un récepteur");
        ...
        do{
            ...
            close(p2[1]);
            read(p2[0],tpr,sz);
            scenarioSNW(&ns,&nr,&finp,&fina,tpe,tpr,adrEm,adrRec);
            
        }while(!finp && !fina);
        puts("Fin de la tâche de l'émetteur");
        waitpid(pid);
    }
    else{
        puts("Je suis le processus fils, je simule le récepteur, mon père est l'émetteur");
        ...
        puts("Fin de la tâche du recepteur");
        _exit(NULL);
    }
    sleep(3);
    outro();
